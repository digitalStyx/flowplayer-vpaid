...
module.exports = {
	attach: function (config) {
		...
		var defaultLanguageConfig = {
			...
			skip: 'Skip Ad',
			unmute: 'Unmute',
			remainingTime: 'The ad will end in : ',
			skipDelay: 'You can skip ads in : '
		}
		...
		vpaidRemainingTime.style.cssText = 'position:absolute;top:0;left:0;width:100%;height:0;visibility:none;overflow:hidden;z-index:4999;display:flex;align-items:center;justify-content:center;';
		var vpaidTimeToSkipContainer = document.createElement('div');
		vpaidTimeToSkipContainer.className = 'vpaid_skipTime';
		vpaidTimeToSkipContainer.style.cssText = 'position:absolute;top:90%;left:0;width:100%;height:0;visibility:none;overflow:hidden;z-index:4999;display:flex;align-items:center;justify-content:center;cursor:pointer;border:white;';
		var vpaidVolumeControlContainer = document.createElement('div');
		vpaidVolumeControlContainer.className = 'vpaid_volume';
		vpaidVolumeControlContainer.style.cssText = 'position:absolute;top:90%;left:80%;width:20%;height:0;visibility:none;overflow:hidden;z-index:4999;display:flex;align-items:center;justify-content:center;cursor:pointer;border:white;';
		...
		var hideVpaid = function () {
			...
			vpaidRemainingTime.style.height = '0';
			vpaidTimeToSkipContainer.style.visibility = 'none';
			vpaidTimeToSkipContainer.style.height = '0';
			vpaidVolumeControlContainer.style.visibility = 'none';
			vpaidVolumeControlContainer.style.height = '0';
		};
		...
		var showVpaid = function () {
			...
			vpaidTimeToSkipContainer.style.visibility = 'visible';
			vpaidTimeToSkipContainer.style.height = '10%';
			vpaidVolumeControlContainer.style.visibility = 'visible';
			vpaidVolumeControlContainer.style.height = '10%';
		};
		var showVpaidSkipAd = function () {
			...
			vpaidSkipAd.innerHTML = '<div id="vpaid-skip"  style="color: white;font-size: 17px;"><strong>' + (config.language.skip ? config.language.skip : defaultLanguageConfig.skip) + '</strong></div>'
		};
			...
			return function (err, unit) {
				...
				vpaidVolumeControlContainer.addEventListener('click', function () {
					if (click === true) {
						...
						vpaidVolumeControlContainer.innerHTML = '<div style="color: white;font-size: 14px;">' + (config.language.mute ? config.language.mute : defaultLanguageConfig.mute) + '</div>'
						click = false
					} else {
						unitConfig.tracker.setMuted(true)
						videoContainer.muted = true
						vpaidVolumeControlContainer.innerHTML = '<div style="color: white;font-size: 14px;">' + (config.language.unmute ? config.language.unmute : defaultLanguageConfig.unmute) + '</div>'
						click = true
					}
		...
		vpaidTime.ontimeupdate = function () {
			...
			if (isNaN(vpaidTimeRemaining)) {
				...
				vpaidRemainingTime.innerHTML = '<div style="color: white;font-size: 16px;">' + (config.language.remainingTime ? config.language.remainingTime : defaultLanguageConfig.remainingTime) + ' ' + vpaidTimeRemaining + ' </div>'
			...
			if (Math.round(vpaidTime.duration) > 15) {
				if (vpaidTimeToSkip >= 0) {
					...
					vpaidTimeToSkipContainer.innerHTML = '<div style="color: white;font-size: 16px;">' + (config.language.skipDelay ? config.language.skipDelay : defaultLanguageConfig.skipDelay) + '' + Math.abs(vpaidTimeToSkip) + ' </div>'
